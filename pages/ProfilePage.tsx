  ...
  if (error && !profileData.id && !pageLoading) {
    return (
        <div className="text-center p-8 text-xl text-red-400 max-w-2xl mx-auto">
            <p>{error}</p>
            <Button onClick={() => history.push('/home')} className="mt-4" variant="outline">Go to Home</Button>
        </div>
    );
  }
  ...
  if (!viewedUserId && !authLoadingHook && !pageLoading) {
    return (
        <div className="text-center p-8 text-xl text-gray-300 max-w-2xl mx-auto">
            ...
            <Button onClick={() => history.push('/login')} className="mt-4" variant="primary">Login</Button>
        </div>
    );
  }
     ...
     return (
        <div className="text-center p-8 text-xl text-gray-300 max-w-2xl mx-auto">
            <p>Could not load profile data for the requested user.</p>
             <Button onClick={() => history.goBack()} leftIcon={<ArrowLeftIcon className="w-5 h-5"/>} className="mt-4" variant="outline"> 
                Go Back
            </Button>
        </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 p-4">
      <div className="flex items-center space-x-3">
        {paramsUserId && <Button variant="outline" size="sm" onClick={() => history.goBack()} leftIcon={<ArrowLeftIcon className="w-5 h-5"/>}>Back</Button>}
