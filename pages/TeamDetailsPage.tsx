...
const TeamDetailsPage = (): JSX.Element => {
  ...
  const [playerModalProfile, setPlayerModalProfile] = useState<UserProfile | null>(null);
    ...
    try {
      ...
      if (fetchedTeam) {
        ...
        if (authUser?.uid) {
          ...
          const memberCheck = !!(currentUserProfile?.teamIds?.includes(teamId) && !ownerCheck);
  ...
  const handlePlayerRosterItemClick = async (playerName: string) => {
    const registeredPlayer = allUserProfiles.find(p => p.username === playerName);
    setSelectedPlayerForModal(registeredPlayer || { username: playerName });
    setIsPlayerModalOpen(true);
    setPlayerModalLoading(true);
    setPlayerModalError(null);
    setPlayerModalProfile(null);

    if (registeredPlayer?.id) {
      try {
        const profile = await getFullUserProfile(registeredPlayer.id);
    ...
    <>
      ...
      {isPlayerModalOpen && (
        <PlayerInfoModal
          ...
          playerUsername={selectedPlayerForModal?.username || null}
          playerProfile={playerModalProfile}
          isLoading={playerModalLoading}
